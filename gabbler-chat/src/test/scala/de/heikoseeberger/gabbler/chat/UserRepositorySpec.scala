...
object UserRepositorySpec {
        ...
        optionalHeaderValueByName(`Last-Event-ID`.name) { lastEventId =>
          try {
            ...
            complete {
              ...
              eventIds.map(toServerSentEvent)
            }
          } catch {
  ...
  class Server(address: String, port: Int, minEventId: Int, maxEventId: Int)
    ...
    private def unbinding(socketAddress: InetSocketAddress): Receive = {
      ...
      case Status.Failure(c) =>
        log.error(c, "Can't unbind from {}!", socketAddress)
        context.stop(self)
    }
  }
...
class UserRepositorySpec extends WordSpec with Matchers with BeforeAndAfterAll {
  ...
  "UserRepository" should {
    "correctly consume user events" in {
      ...
      val server1 = system.actorOf(Props(new Server(host, port, minEventId = 1, maxEventId = 5)))
      probe.watch(server1)
      server1 ! Server.Start
      probe.expectMsg(Server.Started)

      val userRepository1 =
        system.actorOf(UserRepository(Uri(s"http://$host:$port")))
      ...
      val server2 = system.actorOf(Props(new Server(host, port, minEventId = 5, maxEventId = 7)))
      probe.watch(server2)
      server2 ! Server.Start
      probe.expectMsg(Server.Started)

      val userRepository2 = system.actorOf(UserRepository(Uri(s"http://$host:$port")))
      probe.within(10.seconds.dilated) {
        probe.awaitAssert {
